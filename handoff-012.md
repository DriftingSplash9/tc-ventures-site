# handoff-012 — tc-ventures.ca

**Written:** 2026-09-21
**Covers:** a short session clearing the items an agent could close without Thomas:
C-8, most of O-11 (bareyourrare.org markup), and the code half of A-1.
**Status at wrap:** pushed by Thomas and **verified serving** 2026-09-21: tc-ventures.ca (résumé PDF, both new figures, graph keyboard path, contact status), bareyourrare.org (one `MedicalWebPage` and one `BreadcrumbList` on /hcs-guide/, llms.txt link and robots line, privacy meta description, noindex on /category/poems/), and the theme's deleted script (404). **Not deployed:** the O-9 Keg paragraph fix, made after the push.
...
## 2. What was done (2026-09-21)

- **C-8 closed.** `resume/resume-source.html` rewritten to mirror the Word file
  word for word: section order, skills, projects, experience, career note,
  education (C-11 dates, Class 3B, college name). No lander, no Hajdu-Cheney line
  (the Word version does not carry one). Headless Chromium renders it to **two
  letter pages**. The served PDF was not touched.
- **tc-ventures.ca accessibility, code half of A-1.** axe-core found nothing on any
  page in either theme. The fixes were in behaviour axe cannot see:
  - *Graph demo:* the load button hides itself once the graph arrives, which
    dropped keyboard focus to `<body>`. Focus now moves to the stage (or to the
    panel if loading fails). The stage is focusable; **←/→ (or ↑/↓) step through
    the dependencies**, each read into the panel and drawn white and thicker in
    the scene; Esc clears. The camera never moves for it. Panel is
    `aria-live="polite"`. Focus ring sits on `.gd__frame` via `:has()`, because
    the canvas paints over any ring on the stage itself. Tested headless with
    WebGL: focus lands, arrows read out, highlight renders, no console errors.
  - *Contact:* the copy button now announces "Address copied to the clipboard."
    through a `role="status"` span. New `.sr-only` utility in `style.css`.
- **bareyourrare.org, O-11 a/b/d/f** written in `bareyr/` — details in §4.

### Traps worth knowing

- **BYR's `poems-page.php` is not in the include list** in `functions.php` (it is
  commented out) — it is loaded by the `[poems_page]` shortcode through
  `get_template_part()`. Anything it calls must be defined in a file that *is*
  included.
- **Rank Math already emits `BreadcrumbList`** (with `@id`) on every BYR page. Do not
  add a theme copy back.
- **BYR `robots.txt` is WordPress-generated**, not a physical file, so the
  `robots_txt` filter works. If a physical `robots.txt` is ever uploaded, the llms.txt
  line silently disappears.
- **A focus ring on a WebGL stage is invisible** — the canvas covers it. Ring the
  container.
- Every trap in handoff-011 §2 still stands.
...
1. **Thomas pushes the theme** for the O-9 paragraph and purges all three thomascheesman.ca caches; verify /hcs serves it.
2. **D-3:** confirm the Professional Email renewal is paid before 8 October.
3. Pick the next build with Thomas: O-8 (thomascheesman.ca — `three-r128.min.js` idle-loaded on phones that get the Painted Map; mouse wheel over a live stage not scrolling) or O-11g (BYR page weight).
