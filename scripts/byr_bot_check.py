...
for i in range(n):
    ...
    url = "https://bareyourrare.org" + p + (f"?t={time.time_ns()}" if bust else "")
    ...
    print(f"{time.strftime('%H:%M:%S')} {code} {ua:13} {p:22} ls={ls.group(1) if ls else '-':5} cf={cf.group(1) if cf else '-':8} {('BOTVERIFY' if 'Bot Verification' in body else (t.group(1)[:30] if t else ''))}", flush=True)
    time.sleep(gap)
